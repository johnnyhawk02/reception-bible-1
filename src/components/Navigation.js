...
const Navigation = ({ setSidebarOpen }) => {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Header - more compact for portrait */}
      <div className="p-4 lg:p-6 border-b border-gray-100">
        <Link to="/" className="flex items-center" onClick={() => setSidebarOpen && setSidebarOpen(false)}>
          <div>
            <h1 className="font-semibold text-gray-900 text-sm lg:text-base">Reception Bible</h1>
            <p className="text-xs text-gray-500">Meadows Leisure Centre</p>
          </div>
        </Link>
      </div>

      {/* Navigation - more compact spacing for portrait */}
        ...
        <div className="space-y-0.5 lg:space-y-1">
          {navItems.map(item => (
            <Link
              ...
              className={`block w-full px-2 lg:px-3 py-1.5 lg:py-2 text-sm ${
      ...
      {/* Footer - more compact for portrait */}
      <div className="p-2 lg:p-4 border-t border-gray-100">
        <div className="p-2 lg:p-3">
          <div className="text-xs font-medium text-gray-900 mb-1 lg:mb-2">Emergency Contacts</div>
          <div className="space-y-0.5 lg:space-y-1 text-xs text-gray-600">
